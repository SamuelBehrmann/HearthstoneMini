...
case class Field(matrix: Matrix[String] = new Matrix[String](Field.standartFieldHeight, Field.standartFieldWidth, " "),
    ...
    def placeCard( handSlot: Int, fieldSlot: Int): Field = if (players(0).gamebar.mana.value >= players(0).gamebar.hand(handSlot).manaCost) then copy(players = players.updated(0, players(0).placeCard(handSlot ,fieldSlot).reduceMana(players(0).gamebar.hand(handSlot).manaCost))) else this
